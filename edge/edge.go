...
func triggerPythonScript(dirName, edgeID string, percentage int, hyperparams map[string]interface{}, dc *webrtc.DataChannel) error {
	...
	var model *websocket.Conn
	maxRetries := 5
	retryDelay := 2 * time.Second

	for i := 0; i < maxRetries; i++ {
		log.Printf("Connecting to ML model service (Attempt %d/%d)...", i+1, maxRetries)
		var err error
		model, _, err = websocket.DefaultDialer.Dial(ML_MODEL, nil)
		if err == nil {
			log.Println("Successfully connected to ML service")
			break
		}

		if i < maxRetries-1 {
			log.Printf("Error connecting to ML service: %v. Retrying in %v...", err, retryDelay)
			time.Sleep(retryDelay)
			retryDelay *= 2 // Exponential backoff
		} else {
			log.Printf("Failed to connect to ML service after %d attempts: %v", maxRetries, err)
		...
		dc.OnMessage(func(msg webrtc.DataChannelMessage) {
			...
			if msg.IsString {
				...
				if msgText == END {
					...
					log.Printf("Finished receiving file: %s (%d bytes, %d chunks)", file_name, totalBytesReceived, rxChunkCount)
					filesMutex.Lock()
					receivedFiles++
					currentCount := receivedFiles
					filesMutex.Unlock()
					log.Printf("Partitions received in this round: %d of %d", currentCount, totalFiles)
					...
					// Close previous file if still open for some reason
					if f != nil {
						f.Close()
					}
					...
					if err != nil {
						log.Printf("Error while creating file %s: %v", file_path, err)
						return
					}
				}
